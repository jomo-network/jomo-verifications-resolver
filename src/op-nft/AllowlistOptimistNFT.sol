...
contract AllowlistOptimistNFT is ERC721BurnableUpgradeable, ISemver {
    ...
    OptimistAttestationResolver public immutable OPTIMIST_ATTESTATION_RESOLVER;
    ...
    constructor(
        ...
        OptimistAttestationResolver _optimistAllowlist
    ) {
        BASE_URI_ATTESTOR = _baseURIAttestor;
        OPTIMIST_ATTESTATION_RESOLVER = _optimistAllowlist;
    ...
    function mint(address _recipient) public {
        require(isOnAllowList(_recipient), "AllowlistOptimistNFT: address is not on allowList");
    ...
    function baseURI() public view returns (string memory uri_) {
        uri_ = string(
            abi.encodePacked(
                BASE_URI_ATTESTOR, address(this), bytes32("optimist.base-uri")
            )
        );
    }
    ...
    function isOnAllowList(address _recipient) public view returns (bool allowed_) {
        allowed_ = OPTIMIST_ATTESTATION_RESOLVER.hasAttestation(_recipient);
    }
    ...
    function approve(address, uint256) public pure override {
        revert("AllowlistOptimistNFT: soul bound token");
    }
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256) internal virtual {
        require(_from == address(0) || _to == address(0), "AllowlistOptimistNFT: soul bound token");
    }
}